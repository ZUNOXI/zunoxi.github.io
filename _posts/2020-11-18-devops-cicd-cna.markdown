...
---
최근 기존 온프레미스 환경의 인프라에서 클라우드환경으로 시스템 운영환경이 변해가며 클라우드 친화적 개발환경인 `클라우드 네이티브 아키텍쳐(이하 CNA)`에 대한 관심이 높아졌다. 특히나 클라우드가 각광받음과 더불어 `MSA, 컨테이너` 등 이 핫한 기술로 여겨지고 있다. 이번 포스팅에서는 CNA에 대한 개념을 잡고 CNA의 핵심인 여러 개념들에대해 알아보려한다.
